...
struct FractureQuantities {
  ...
  FractureQuantities(TPZMultiphysicsCompMesh* cmesh, const std::set<int>& allfracs, int pressureintersect, int matglue) : fCMesh(cmesh), fAllFracturesMatId(allfracs), fPressureIntersectMatid(pressureintersect), fGlueMatid(matglue) {
  }
  ...
  void ComputeTransmissionBreakdown() {
    ...
    for (int64_t el = 0; el < nel; el++) {
      ...
      REAL flux = fluxvec[0];
      if (flux < 0.)
        previntegral.first += flux;
      else
        previntegral.second += flux;
      fBoundaryTransmission[fracmats] = previntegral;
    }
  }
};
...
void ModifyBCsForCase3(TPZGeoMesh* gmesh) {
  ...
  for (auto gel : gmesh->ElementVec()) {
    ...
    if ((gel->MaterialId() != ENoflux) && (gel->MaterialId() != EInlet) && (gel->MaterialId() != EOutlet)) {
      continue;
    }
    TPZVec<REAL> masscent(2, 0.0), xcenter(3, 0.0);
    gel->CenterPoint(gel->NSides() - 1, masscent);
...
void CreateOutputFolders(std::string& outputFolder) {
  ...
  while (folders.find("/") != std::string::npos) {
    if (nfolders == 0)
      folderToCreate = folders.substr(0, folders.find("/"));
    else
      folderToCreate = folderToCreate + "/" + folders.substr(0, folders.find("/"));
